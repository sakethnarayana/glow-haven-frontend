...
// const BUTTON_BASE = 'inline-flex items-center justify-center gap-2 px-3 py-1.5 rounded-lg font-semibold transition-all duration-150';

// const ProductCard = ({ item, clickable = false }) => {
//   const navigate = useNavigate();
//   const { isAuthenticated } = useAuth();
//   const { cartItems, addItem, updateQuantity } = useCart();

//   // derive quantity from global cart
//   const findInCart = cartItems.find((c) => c._id === item._id);
//   const qtyFromCart = findInCart ? (findInCart.quantity || 0) : 0;

//   // Local optimistic UI state derived from cart
//   const [localQty, setLocalQty] = useState(qtyFromCart);
//   useEffect(() => setLocalQty(qtyFromCart), [qtyFromCart]);

//   const onCardClick = () => {
//     if (clickable) navigate(`/products?highlight=${item._id}`);
//   };
...
//     addItem(item, 1); // CartProvider updates persist and will reflect back in quantity
//   };
...
//     if (final === 0) {
//       updateQuantity(item._id, 0); // will remove item
//     } else {
...
//           <img src={item.image || '/placeholder.jpg'} alt={item.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" onError={(e)=>e.currentTarget.src='https://via.placeholder.com/400x300?text=Product+Image'} />
//           {typeof item.stock !== 'undefined' && (
//             <div className="absolute top-3 left-3">
//               {item.stock > 0 ? <span className="px-2 py-1 bg-green-500 text-white text-xs rounded-full">In Stock</span> : <span className="px-2 py-1 bg-red-500 text-white text-xs rounded-full">Out of Stock</span>}
//             </div>
//           )}
//         </div>
...
//           <div className="mb-2">
//             {item.category && <span className="inline-block px-2 py-0.5 bg-purple-100 text-purple-700 text-xs rounded-full mb-2">{item.category}</span>}
//             <h3 className="text-base md:text-lg font-semibold text-gray-900 mb-1 line-clamp-1">{item.name}</h3>
//             <p className="text-sm text-gray-600 line-clamp-2">{item.description || 'Premium quality product for your beauty needs'}</p>
//           </div>

//           <div className="mt-auto pt-3 border-t border-gray-100 flex items-center justify-between gap-3">
//             <div>
//               <p className="text-lg md:text-2xl font-bold text-gray-900">₹{item.price}</p>
//               {typeof item.stock !== 'undefined' && <p className="text-xs text-gray-500">{item.stock} units left</p>}
//             </div>

//             <div className="flex items-center">
//               {localQty <= 0 ? (
//                 <button onClick={handleAdd} className={`${BUTTON_BASE} bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-md`} aria-label="Add to cart">
//                   <ShoppingCart className="w-4 h-4" />
//                   <span className="text-sm hidden md:inline">Add</span>
//                 </button>
//               ) : (
//                 <div className="flex items-center gap-2 bg-gray-100 rounded-lg px-2 py-1">
//                   <button onClick={(e)=>handleChangeQty(e, localQty - 1)} className="p-1 rounded hover:bg-gray-200 active:scale-95"><Minus className="w-4 h-4" /></button>
//                   <div className="min-w-[36px] text-center font-semibold text-gray-800">{localQty}</div>
//                   <button onClick={(e)=>handleChangeQty(e, localQty + 1)} className="p-1 rounded hover:bg-gray-200 active:scale-95"><Plus className="w-4 h-4" /></button>
//                 </div>
//               )}
//             </div>
//           </div>
...
//   setTimeout(() => toast.remove('open-product'), 900);
// };
//   const openAuth = () => window.dispatchEvent(new Event('app:openAuth'));
